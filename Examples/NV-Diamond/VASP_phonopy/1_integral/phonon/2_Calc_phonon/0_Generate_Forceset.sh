...
if [ -e "$DIR_force/$dir/vasprun.xml" ];then
  Str_disp+="$DIR_force/$dir/vasprun.xml "
else
  echo "no vasprun.xml in $dir, please double check the calculation"
  exit 1
fi
done
